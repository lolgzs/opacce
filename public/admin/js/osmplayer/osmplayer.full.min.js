...
(function(a,b){var c;a:{for(var d=[["requestFullscreen","exitFullscreen","fullscreenchange","fullscreen","fullscreenElement"],["webkitRequestFullScreen","webkitCancelFullScreen","webkitfullscreenchange","webkitIsFullScreen","webkitCurrentFullScreenElement"],["mozRequestFullScreen","mozCancelFullScreen","mozfullscreenchange","mozFullScreen","mozFullScreenElement"]],e=0,h=d.length;e<h;e++)if(d.hasOwnProperty(e)){var i=d[e];if(i[1]in b){c=i;break a}}c=void 0}if(!c)return a.screenfull=!1;var k="ALLOW_KEYBOARD_INPUT"in
Element,j={init:function(){b.addEventListener(c[2],function(a){j.isFullscreen=b[c[3]];j.element=b[c[4]];j.onchange(a)});return this},isFullscreen:b[c[3]],element:b[c[4]],request:function(a){a=a||b.documentElement;a[c[0]](k&&Element.ALLOW_KEYBOARD_INPUT);if(!b.isFullscreen)a[c[0]]()},exit:function(){b[c[1]]()},toggle:function(a){this.isFullscreen?this.exit():this.request(a)},onchange:function(){}};a.screenfull=j.init()})(window,document);
...
minplayer.playLoader.prototype.loadPreview=function(){this.enabled&&this.elements.preview&&(this.options.preview?(this.elements.preview.addClass("has-preview").show(),this.preview=new minplayer.image(this.elements.preview,this.options),this.preview.load(this.options.preview)):this.elements.preview.hide())};
...
minplayer.players.base.prototype.clear=function(){this.playerReady=!1;this.reset();this.player&&jQuery(this.player).unbind()};
...
window.onFlashPlayerDebug=function(a){console&&console.log&&console.log(a)};minplayer.players.minplayer.getPriority=function(){return 1};minplayer.players.minplayer.canPlay=function(a){return!(0<=jQuery.inArray(a.mimetype,["video/x-webm","video/webm","application/octet-stream"]))&&("video"==a.type||"audio"==a.type)};
...
(function(){var a=Math,b=/webkit/i.test(navigator.appVersion)?"webkit":/firefox/i.test(navigator.userAgent)?"Moz":/trident/i.test(navigator.userAgent)?"ms":"opera"in window?"O":"",c=/android/gi.test(navigator.appVersion),d=/iphone|ipad/gi.test(navigator.appVersion),e=/playbook/gi.test(navigator.appVersion),h=/hp-tablet/gi.test(navigator.appVersion),i="WebKitCSSMatrix"in window&&"m11"in new WebKitCSSMatrix,k="ontouchstart"in window&&!h,j=b+"Transform"in document.documentElement.style,l=d||e,r=window.requestAnimationFrame||
window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame||function(a){return setTimeout(a,1)},t=window.cancelRequestAnimationFrame||window.webkitCancelAnimationFrame||window.webkitCancelRequestAnimationFrame||window.mozCancelRequestAnimationFrame||window.oCancelRequestAnimationFrame||window.msCancelRequestAnimationFrame||clearTimeout,s="onorientationchange"in window?"orientationchange":"resize",x=k?"touchstart":"mousedown",
u=k?"touchmove":"mousemove",v=k?"touchend":"mouseup",w=k?"touchcancel":"mouseup",y="Moz"==b?"DOMMouseScroll":"mousewheel",n="translate"+(i?"3d(":"("),p=i?",0)":")",e=function(a,f){var g=this,e=document,q;g.wrapper="object"==typeof a?a:e.getElementById(a);g.wrapper.style.overflow="hidden";g.scroller=g.wrapper.children[0];g.options={hScroll:!0,vScroll:!0,x:0,y:0,bounce:!0,bounceLock:!1,momentum:!0,lockDirection:!0,useTransform:!0,useTransition:!1,topOffset:0,checkDOMChanges:!1,hScrollbar:!0,vScrollbar:!0,
fixedScrollbar:c,hideScrollbar:d,fadeScrollbar:d&&i,scrollbarClass:"",zoom:!1,zoomMin:1,zoomMax:4,doubleTapZoom:2,wheelAction:"scroll",snap:!1,snapThreshold:1,onRefresh:null,onBeforeScrollStart:function(a){a.preventDefault()},onScrollStart:null,onBeforeScrollMove:null,onScrollMove:null,onBeforeScrollEnd:null,onScrollEnd:null,onTouchEnd:null,onDestroy:null,onZoomStart:null,onZoom:null,onZoomEnd:null};for(q in f)g.options[q]=f[q];g.x=g.options.x;g.y=g.options.y;g.options.useTransform=j?g.options.useTransform:
...
x:0,y:0,steps:[],scale:1,currPageX:0,currPageY:0,pagesX:[],pagesY:[],aniTime:null,wheelZoomCount:0,handleEvent:function(a){switch(a.type){case x:if(!k&&0!==a.button)break;this._start(a);break;case u:this._move(a);break;case v:case w:this._end(a);break;case s:this._resize();break;case y:this._wheel(a);break;case "mouseout":this._mouseout(a);break;case "webkitTransitionEnd":this._transitionEnd(a)}},_checkDOMChanges:function(){!this.moved&&(!this.zoomed&&!(this.animating||this.scrollerW==this.scroller.offsetWidth*
...
"Transform"]=n+("h"==a?g+"px,0":"0,"+g+"px")+p)},_start:function(c){var f=k?c.touches[0]:c,g,d;if(this.enabled){this.options.onBeforeScrollStart&&this.options.onBeforeScrollStart.call(this,c);(this.options.useTransition||this.options.zoom)&&this._transitionTime(0);this.zoomed=this.animating=this.moved=!1;this.dirY=this.dirX=this.absDistY=this.absDistX=this.distY=this.distX=0;this.options.zoom&&(k&&1<c.touches.length)&&(d=a.abs(c.touches[0].pageX-c.touches[1].pageX),g=a.abs(c.touches[0].pageY-c.touches[1].pageY),
...
""),g=1*getComputedStyle(this.scroller,null).top.replace(/[^0-9-]/g,"")),d!=this.x||g!=this.y))this.options.useTransition?this._unbind("webkitTransitionEnd"):t(this.aniTime),this.steps=[],this._pos(d,g);this.absStartX=this.x;this.absStartY=this.y;this.startX=this.x;this.startY=this.y;this.pointX=f.pageX;this.pointY=f.pageY;this.startTime=c.timeStamp||Date.now();this.options.onScrollStart&&this.options.onScrollStart.call(this,c);this._bind(u);this._bind(v);this._bind(w)}},_move:function(c){var f=k?
c.touches[0]:c,g=f.pageX-this.pointX,d=f.pageY-this.pointY,e=this.x+g,h=this.y+d,i=c.timeStamp||Date.now();this.options.onBeforeScrollMove&&this.options.onBeforeScrollMove.call(this,c);if(this.options.zoom&&k&&1<c.touches.length)e=a.abs(c.touches[0].pageX-c.touches[1].pageX),h=a.abs(c.touches[0].pageY-c.touches[1].pageY),this.touchesDist=a.sqrt(e*e+h*h),this.zoomed=!0,f=1/this.touchesDistStart*this.touchesDist*this.scale,f<this.options.zoomMin?f=0.5*this.options.zoomMin*Math.pow(2,f/this.options.zoomMin):
...
this.x,this.startY=this.y),this.options.onScrollMove&&this.options.onScrollMove.call(this,c))}},_end:function(c){if(!(k&&0!=c.touches.length)){var f=this,d=k?c.changedTouches[0]:c,e,q,h={dist:0,time:0},i={dist:0,time:0},j=(c.timeStamp||Date.now())-f.startTime,m=f.x,l=f.y;f._unbind(u);f._unbind(v);f._unbind(w);f.options.onBeforeScrollEnd&&f.options.onBeforeScrollEnd.call(f,c);if(f.zoomed)m=f.scale*f.lastScale,m=Math.max(f.options.zoomMin,m),m=Math.min(f.options.zoomMax,m),f.lastScale=m/f.scale,f.scale=
m,f.x=f.originX-f.originX*f.lastScale+f.x,f.y=f.originY-f.originY*f.lastScale+f.y,f.scroller.style[b+"TransitionDuration"]="200ms",f.scroller.style[b+"Transform"]=n+f.x+"px,"+f.y+"px"+p+" scale("+f.scale+")",f.zoomed=!1,f.refresh(),f.options.onZoomEnd&&f.options.onZoomEnd.call(f,c);else{if(f.moved){if(300>j&&f.options.momentum){h=m?f._momentum(m-f.startX,j,-f.x,f.scrollerW-f.wrapperW+f.x,f.options.bounce?f.wrapperW:0):h;i=l?f._momentum(l-f.startY,j,-f.y,0>f.maxScrollY?f.scrollerH-f.wrapperH+f.y-f.minScrollY:
0,f.options.bounce?f.wrapperH:0):i;m=f.x+h.dist;l=f.y+i.dist;if(0<f.x&&0<m||f.x<f.maxScrollX&&m<f.maxScrollX)h={dist:0,time:0};if(f.y>f.minScrollY&&l>f.minScrollY||f.y<f.maxScrollY&&l<f.maxScrollY)i={dist:0,time:0}}h.dist||i.dist?(h=a.max(a.max(h.time,i.time),10),f.options.snap&&(i=m-f.absStartX,j=l-f.absStartY,a.abs(i)<f.options.snapThreshold&&a.abs(j)<f.options.snapThreshold?f.scrollTo(f.absStartX,f.absStartY,200):(i=f._snap(m,l),m=i.x,l=i.y,h=a.max(i.time,h))),f.scrollTo(m>>0,l>>0,h)):f.options.snap?
(i=m-f.absStartX,j=l-f.absStartY,a.abs(i)<f.options.snapThreshold&&a.abs(j)<f.options.snapThreshold?f.scrollTo(f.absStartX,f.absStartY,200):(i=f._snap(f.x,f.y),(i.x!=f.x||i.y!=f.y)&&f.scrollTo(i.x,i.y,i.time))):f._resetPos(200)}else k&&(f.doubleTapTimer&&f.options.zoom?(clearTimeout(f.doubleTapTimer),f.doubleTapTimer=null,f.options.onZoomStart&&f.options.onZoomStart.call(f,c),f.zoom(f.pointX,f.pointY,1==f.scale?f.options.doubleTapZoom:1),f.options.onZoomEnd&&setTimeout(function(){f.options.onZoomEnd.call(f,
c)},200)):f.doubleTapTimer=setTimeout(function(){f.doubleTapTimer=null;for(e=d.target;1!=e.nodeType;)e=e.parentNode;"SELECT"!=e.tagName&&("INPUT"!=e.tagName&&"TEXTAREA"!=e.tagName)&&(q=document.createEvent("MouseEvents"),q.initMouseEvent("click",!0,!0,c.view,1,d.screenX,d.screenY,d.clientX,d.clientY,c.ctrlKey,c.altKey,c.shiftKey,c.metaKey,0,null),q._fake=!0,e.dispatchEvent(q))},f.options.zoom?250:0)),f._resetPos(200);f.options.onTouchEnd&&f.options.onTouchEnd.call(f,c)}}},_resetPos:function(a){var c=
0<=this.x?0:this.x<this.maxScrollX?this.maxScrollX:this.x,d=this.y>=this.minScrollY||0<this.maxScrollY?this.minScrollY:this.y<this.maxScrollY?this.maxScrollY:this.y;c==this.x&&d==this.y?(this.moved&&(this.moved=!1,this.options.onScrollEnd&&this.options.onScrollEnd.call(this)),this.hScrollbar&&this.options.hideScrollbar&&("webkit"==b&&(this.hScrollbarWrapper.style[b+"TransitionDelay"]="300ms"),this.hScrollbarWrapper.style.opacity="0"),this.vScrollbar&&this.options.hideScrollbar&&("webkit"==b&&(this.vScrollbarWrapper.style[b+
"TransitionDelay"]="300ms"),this.vScrollbarWrapper.style.opacity="0")):this.scrollTo(c,d,a||0)},_wheel:function(a){var b=this,c,d;if("wheelDeltaX"in a)c=a.wheelDeltaX/12,d=a.wheelDeltaY/12;else if("wheelDelta"in a)c=d=a.wheelDelta/12;else if("detail"in a)c=d=3*-a.detail;else return;"zoom"==b.options.wheelAction?(d=b.scale*Math.pow(2,1/3*(d?d/Math.abs(d):0)),d<b.options.zoomMin&&(d=b.options.zoomMin),d>b.options.zoomMax&&(d=b.options.zoomMax),d!=b.scale&&(!b.wheelZoomCount&&b.options.onZoomStart&&
...
function(a){return function(b){b.preventDefault();var c=a.display.offset()[a.orient.offset];a.mousePos=b[a.orient.pagePos];a.mousePos-=c}}(this)).bind("mouseenter",function(a){return function(b){b.preventDefault();a.scrolling=!0;var c=function(){if(a.scrolling){var b=a.scroll[a.orient.wrapperSize]/2,d=a.mousePos-b;if(Math.abs(d)>a.options.hysteresis){var h=a.options.hysteresis,d=a.options.scrollSpeed*(d+h*(0<d?-1:0)),d=d/b,b=a.scroll[a.orient.pos]-d,h=a.scroll[a.orient.minScroll]||0,r=a.scroll[a.orient.maxScroll];
...
osmplayer);
(function(a,b){b=b||{};b.playlist=b.playlist||{};b.playlist[a]=function(a,d){b.playlist.call(this,a,d)};b.playlist[a].prototype=new b.playlist;b.playlist[a].prototype.constructor=b.playlist[a];b.playlist[a].prototype.construct=function(){this.options=jQuery.extend({showPlaylist:!0},this.options);b.playlist.prototype.construct.call(this);minplayer.showThenHide(this.elements.hideShow);this.get("player",function(a){var b=this.options.vertical?"width":"height",e=this.options.vertical?"right":"bottom",
h=this.options.vertical?"marginRight":"marginBottom";this.hideShow=function(i,k){var j={},l={},r=this.display[b](),t=this.options.vertical?"e":"s",s=this.options.vertical?"w":"n";jQuery("span",this.elements.hideShow).removeClass(i?"ui-icon-triangle-1-"+s:"ui-icon-triangle-1-"+t).addClass(i?"ui-icon-triangle-1-"+t:"ui-icon-triangle-1-"+s);j[e]=i?r:0;a.elements.minplayer&&(k?a.elements.minplayer.animate(j,"fast"):a.elements.minplayer.css(j));l[h]=i?0:-r;k?this.display.animate(l,"fast",function(){a.resize()}):
this.display.css(l)};this.bind("playlistLoad",function(a){return function(b,c){1===c.nodes.length?a.hideShow(!1,!0):a.hideShow(!0,!0)}}(this));this.elements.hideShow&&this.elements.hideShow.bind("click",function(a){return function(b){b.preventDefault();b=jQuery("span",a.elements.hideShow).hasClass("ui-icon-triangle-1-"+(a.options.vertical?"w":"n"));a.hideShow(b,!0)}}(this));a.elements.minplayer&&(this.options.showPlaylist?this.options.vertical?a.elements.minplayer.css("right",this.display.width()+
...
a+'-playlist-list"></div>      </div>      </div>');return jQuery(".osmplayer-"+a+"-playlist",this.context)};b.playlist[a].prototype.getElements=function(){var c=b.playlist.prototype.getElements.call(this),d=this.options.vertical?"playlist-vertical":"playlist-horizontal",d=d+(this.options.playlistOnly?" playlist-only":""),e=this.options.showPlaylist,e=this.options.vertical?e?"e":"w":e?"s":"n",h=this.options.vertical?"ui-corner-left":"ui-corner-top";(this.options.disablePlaylist||!this.options.playlist)&&
this.display.remove();this.display.addClass(d);d=jQuery(".osmplayer-"+a+"-hide-show-playlist",this.display);d.addClass(h);this.options.playlistOnly&&(d.hide(),d=null);jQuery("span",d).addClass("ui-icon-triangle-1-"+e);return jQuery.extend(c,{playlist_busy:jQuery(".minplayer-"+a+"-loader-wrapper",this.display),list:jQuery(".osmplayer-"+a+"-playlist-list",this.display),scroll:jQuery(".osmplayer-"+a+"-playlist-scroll",this.display),hideShow:d})}})("default",osmplayer);
(function(a,b){b=b||{};b.teaser=b.teaser||{};b.teaser[a]=function(a,d){b.teaser.call(this,a,d)};b.teaser[a].prototype=new b.teaser;b.teaser[a].prototype.constructor=b.teaser[a];b.teaser[a].prototype.construct=function(){minplayer.display.prototype.construct.call(this);this.display.bind("mouseenter",function(a){return function(){a.addClass("ui-state-hover")}}(this.elements.info)).bind("mouseleave",function(a){return function(){a.removeClass("ui-state-hover")}}(this.elements.info))};b.teaser[a].prototype.getDisplay=
...
b.teaser[a].prototype.getElements=function(){var c=b.teaser.prototype.getElements.call(this);return jQuery.extend(c,{info:jQuery(".osmplayer-"+a+"-teaser-info",this.display),title:jQuery(".osmplayer-"+a+"-teaser-title",this.display),image:jQuery(".osmplayer-"+a+"-teaser-image",this.display)})}})("default",osmplayer);
