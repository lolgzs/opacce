      ...
      foreach($filesDir as $file) {
        if(is_dir($current_path . $file)) {
          ...
          if(!isset($this->params['type']) || 
						 (isset($this->params['type']) && strtolower($this->params['type'])=='images' && in_array(strtolower($this->item['filetype']),$this->config['images'])) ||
						 (isset($this->params['type']) && ($this->item['filetype'] == 'swf'))) {

            if($this->config['upload']['imagesonly']== false || 
							 ($this->config['upload']['imagesonly']== true && in_array(strtolower($this->item['filetype']),$this->config['images']))) {
              $array[$this->get['path'] . $file] = array(
							'Path'=>$this->get['path'] . $file,
							'Filename'=>$this->item['filename'],
							'File Type'=>$this->item['filetype'],
  ...
  public function add() {
    ...
    if(!isset($_FILES['newfile']) || !is_uploaded_file($_FILES['newfile']['tmp_name'])) {
      $this->error(sprintf($this->lang('INVALID_FILE_UPLOAD')),true);
		...
		$is_type_image = ($this->config['upload']['imagesonly'] || (isset($this->params['type']) && strtolower($this->params['type'])=='images'));
		$max_size = $is_type_image ? $this->config['upload']['images']['size'] : $this->config['upload']['size'];

    if(($max_size!=false && is_numeric($max_size)) && ($_FILES['newfile']['size'] > ($max_size * 1024 * 1024))) {
      $this->error(sprintf($this->lang('UPLOAD_FILES_SMALLER_THAN'), $max_size . 'Mb'),true);
    }

    if ( $is_type_image ) {
			$extension = end(explode('.', $_FILES['newfile']['name']));

			if((!$size = @getimagesize($_FILES['newfile']['tmp_name'])) and ($extension !== 'ico')){
        $this->error(sprintf($this->lang('UPLOAD_IMAGES_ONLY')),true);
      }
			
			if ((!in_array($size[2], array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG))) and ($extension !== 'ico')) {
