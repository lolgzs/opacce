 ...
 */
//////////////////////////////////////////////////////////////////////////////////////////
// OPAC3 - AMAZON : DOCUMENTS VIDEO
//////////////////////////////////////////////////////////////////////////////////////////

class Class_WebService_AmazonVideo extends Class_WebService_Amazon
{
...
//------------------------------------------------------------------------------------------------------
	...
	{
		$req=$this->req_ean_video($ean);
		if($req == "") return false;
		$req.="&ResponseGroup=Large";
		...
		if($ret["nb_resultats"] >0 )
		{
			...
			// resume
			$node_resume=$this->xml->getNode("EDITORIALREVIEW");
			if($node_resume >0) $ret["resume"]=$this->xml->get_child_value($node_resume,"content");
			// auteurs
...
//------------------------------------------------------------------------------------------------------
	public function getImages($ean)
	{
		...
		$req=$this->req_ean_video($ean)."&ResponseGroup=Images";
		if(!$this->requete($req)) return false;
...
//------------------------------------------------------------------------------------------------------
	function req_ean_video($ean)
	{
		if(strlen($ean) != 13) return "";
		$isbn = str_replace("-","",$ean);
		$url="&Operation=ItemSearch&SearchIndex=Video&Keywords=" .$ean;
		return $url;
	}
}