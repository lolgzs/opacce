...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	var self, opts, b, src,
		...
		chapter, dd, ddUL, ddH, ddLI, ddA, ddT, ddC, ddCUL, ddCH, ddCLI, ddCA, ddCT,
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//setup menu
	if(opts.menu){
		if (opts.menu.jquery == undefined) {
			opts.menu = $(opts.menu);
		}
		opts.menu.addClass('b-menu');
		p = opts.curr;		
		//setup page selctor
		if(opts.pageSelector){
			//add selector
			dd = $('<div class="b-selector b-selector-page"><span class="b-current">'+ (p+1) +' - '+ (p+2) +'</span></div>').appendTo(opts.menu);
		...
		if(opts.chapterSelector){
			...
			ddC = $('<div class="b-selector b-selector-chapter"><span class="b-current">'+chapter+'</span></div>').appendTo(opts.menu);
			...
			initDropDownMenu(ddC, ddCUL);
		}
	}


	function initDropDownMenu(container, menuList) {
		var fullHeight = menuList.height();
		menuList.css({'height':0, 'padding-bottom':0});
		container
			.unbind('hover')
			.unbind('click')
			.click(function(){
				if (menuList.height() == 0)
						menuList.stop().animate({height:fullHeight, paddingBottom:10}, 500);
				else
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//add overlays
	if(opts.overlays){
		overlayP = $('<div class="b-overlay b-overlay-prev b-prev" title="' + opts.previousPageTitle + '"></div>').appendTo(b);
		overlayN = $('<div class="b-overlay b-overlay-next b-next" title="' + opts.nextPageTitle + '"></div>').appendTo(b);
	...
	//add tabs
	if(opts.tabs){
		tabP = $('<div class="b-tab b-tab-prev b-prev" title="'+ opts.previousPageTitle + '">Previous</div>').appendTo(b);
		tabN = $('<div class="b-tab b-tab-next b-next" title="'+ opts.nextPageTitle + '">Next</div>').appendTo(b);
		tabs = b.find('.b-tab');
		...
		if(opts.direction == 'RTL'){
			tabN.html('Previous').attr('title', opts.previousPageTitle);
			tabP.html('Next').attr('title', opts.nextPageTitle);
		}
	}else{
		b.css({'marginTop': 0});
	}
	...
	//add arrows
	if(opts.arrows){
		arrowP = $('<div class="b-arrow b-arrow-prev b-prev" title="'+ opts.previousPageTitle + '"><div>'+ opts.previousPageTitle + '</div></div>').appendTo(b);
		arrowN = $('<div class="b-arrow b-arrow-next b-next" title="'+ opts.nextPageTitle + '"><div>'+ opts.nextPageTitle + '</div></div>').appendTo(b);
		arrows = b.find('.b-arrow');
		
		//update ctrls for RTL direction
		if(opts.direction == 'RTL'){
			arrowN.html('<div>'+ opts.previousPageTitle + '</div>').attr('title',opts.previousPageTitle);
			arrowP.html('<div>'+ opts.nextPageTitle + '</div>').attr('title', opts.nextPageTitle);
		}
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function updateMenuText(text) {
		if (undefined == opts.menu.jquery)
			return;

		$(opts.menu).find('.b-current').text(text);
	}
	...
	function updatePager(){
		...
		if(opts.chapterSelector){
			if(chapters[opts.curr]!=""){
				updateMenuText(chapters[opts.curr]);
			}else if(chapters[opts.curr+1]!=""){
				updateMenuText(chapters[opts.curr+1]);
			} else {
				for(i=opts.curr; i>0; i=i-1) {
					if (chapters[i] != "") {
						updateMenuText(chapters[i]);
						break;
					}
				}
			}
...
$.fn.booklet.defaults = {
	...
	previousPageTitle:  'Previous page',                 // title displayed on overlay and tab for previous page
	nextPageTitle:      'Next page',                     // title displayed on overlay and tab for next page
