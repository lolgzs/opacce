...
if($phase == 11)
{
	if(!$mode_cron and $phase_data["nombre"]>0) print("<h4>Intégration des fichiers de prêts</h4>");
	...
	while($ligne=$sql->fetchNext($resultat))
	{
		...
		if($type_fichier != 2 ) continue; // Si autre que prêts on passe
		...
		while(true)
		{
			...
			if($ret["statut"]=="ok")
			{
				if(substr($ret["data"],0,14)=='BIB_PRET_COURS') continue; // Entete pergame
				$transac->importFichePret($ret["data"]);
				$phase_data["nombre"]++;
...
// ----------------------------------------------------------------
function traceTraitementTransaction()
{
